...
import ExamplePrompts, {ExamplePrompt} from './components/ExamplePrompts';
...
const examplePrompts: ExamplePrompt[] = [
  {
    ...
    config: {
      prompt:
        "A funny, social media style video, like an Instagram Reel. A female host on a street in Karachi, Pakistan, asks young men: 'What's one thing you'd hide from your girlfriend?' One guy nervously says he's at the gym when he's with friends. Another jokes he doesn't have a girlfriend to avoid stress. The video ends with the host smiling at the camera, asking 'Which answer was the best? Comment below!'. Upbeat, vibrant, and humorous.",
  ...
  },
  {
    ...
    config: {
      prompt:
        'An epic cinematic drone shot flying through a misty, ancient forest. Sunlight breaks through the canopy, illuminating a hidden waterfall cascading into a turquoise pool. Hyperrealistic, 8k, majestic fantasy style.',
  ...
  },
  {
    ...
    config: {
      prompt:
        'A sleek, futuristic cyber-cat with glowing neon circuits stealthily prowls through a rain-slicked alley in Neo-Tokyo. The city is filled with towering holographic advertisements. Blade Runner aesthetic, moody, atmospheric.',
      aspectRatio: AspectRatio.LANDSCAPE,
      resolution: Resolution.P720,
      model: VeoModel.VEO_FAST,
    },
...
const App: React.FC = () => {
  ...
  const handleSelectExample = useCallback(
    (config: Partial<GenerateVideoParams>) => {
      // Merge with default values to ensure all fields are set
      const defaults: GenerateVideoParams = {
        prompt: '',
        model: VeoModel.VEO_FAST,
        aspectRatio: AspectRatio.LANDSCAPE,
        resolution: Resolution.P720,
      ...
      setInitialFormValues({...defaults, ...config});
    },
    [],
  );
